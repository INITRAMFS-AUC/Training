"""
train_4cmd.py — Retrain for 4 commands: go, left, right, stop.

Strategy:
  1. Load mel_compact_4blk_ch36_int8norm.h5 (11-class backbone)
  2. Discard dense_out + softmax; attach new Dense(5) + Softmax head
  3. Phase 1: freeze backbone, train head only (20 epochs)
  4. Phase 2: unfreeze all, fine-tune end-to-end (cosine LR, patience=40)
  5. PTQ export with KLD calibration using pi_dataset + GSCD

Classes (sorted): go=0, left=1, right=2, stop=3, unknown=4
"""
...
# ── paths ─────────────────────────────────────────────────────────────────────
CKPT_IN      = 'mel_compact_4blk_ch36_int8norm.h5'
CKPT_OUT     = 'mel_compact_4cmd_int8norm.h5'
WEIGHTS_OUT  = 'weights_4cmd.h'
BARBARY_DIR  = 'newdatafinalbarbary/dataset'
GSCD_DIR     = '/workspace/Desktop/Models/Ziad/dataset'
PI_DIR       = '/workspace/Desktop/Main/pi_dataset'

# ── label map ─────────────────────────────────────────────────────────────────
CORE_COMMANDS = ['go', 'left', 'right', 'stop']
...
SAMPLES  = 8000
BATCH    = 64
LR_MAX   = 5e-5   # small — backbone already well-trained
LR_MIN   = 1e-6
EPOCHS   = 150
PATIENCE = 40
...
def load_barbary_dataset(rng):
    ...
    for folder in sorted(os.listdir(BARBARY_DIR)):
        ...
        if label_str is None: continue   # skip keywords not in our 4
...
# ── pi_dataset loader (for calibration + eval) ────────────────────────────────
def load_pi_clips(cmd):
    folder = os.path.join(PI_DIR, cmd)
    clips, labels = [], []
    for f in sorted(os.listdir(folder)):
        if not f.endswith('_16k_s16_monitor.wav'): continue
...
if __name__ == '__main__':
    ...
    # 1. Load GSCD filtered to 4 commands
    print("Loading GSCD (4 commands)...")
    ...
    # 2. Load Barbary (only go/left/right/stop folders used)
    ...
    # 4. Build 5-class model from 11-class backbone
    ...
    # Backbone ends at dropout (layer 22); discard dense_out + softmax
    backbone_out = base.get_layer('dropout').output
    x = tf.keras.layers.Dense(NUM_CLASSES, name='dense_out_4cmd')(backbone_out)
    x = tf.keras.layers.Softmax(name='softmax_4cmd')(x)
    model = tf.keras.Model(inputs=base.input, outputs=x)

    # 5. Phase 1: freeze backbone, train only new head
    print("\nPhase 1: train head only (20 epochs)...")
    for layer in model.layers:
        layer.trainable = (layer.name in ('dense_out_4cmd', 'softmax_4cmd'))
    model.compile(
    ...
    print(f"\nFine-tuned — GSCD: {acc_gscd*100:.2f}%  Barbary: {acc_barb*100:.2f}%")
    ...
    # 8. Load pi_dataset for eval + calibration
    ...
    print(f"\nDone. Model: {CKPT_OUT}  Weights: {WEIGHTS_OUT}")
