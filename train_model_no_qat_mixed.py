...
class RunConfig:
    ...
    pi_data_dir: str = "/workspace/Desktop/Main/pi_dataset"
    ...
    # Pi Dataset Oversampling (Applied ONLY to training set)
    pi_oversample_factor: int = 10
...
# ══════════════════════════════════════════════════════════════════════════
def peak_normalize_int16(samples: np.ndarray, target_peak: int = TARGET_PEAK) -> np.ndarray:
    current_peak = int(np.max(np.abs(samples.astype(np.int32))))
    if current_peak == 0:
        return samples
    scale = float(target_peak) / float(current_peak)
    return np.clip(
        np.round(samples.astype(np.float32) * scale), -32768, 32767
    ).astype(np.int16)

def process_pi_split(x_split_raw, target_dtype):
    """Applies peak norm, downsampling, and bit-shifting to a raw dataset split."""
    x_processed = []
    for audio in x_split_raw:
        if audio.ndim > 1:
            audio = np.squeeze(audio)
            
        # Step A: Peak normalize
        audio_norm = peak_normalize_int16(audio)
        # Step B: Downsample by 2
        audio_down = audio_norm[::2]
        
        # Step C: Match target data type of the main dataset
        if target_dtype == np.float32:
            audio_casted = audio_down.astype(np.float32) / 32768.0
        else:
            audio_casted = audio_down.astype(target_dtype)
            
        x_processed.append(audio_casted)
        
    x_proc = np.array(x_processed)
    # Add channel dimension back if needed (N, 8000) -> (N, 8000, 1)
    if x_proc.ndim == 2:
...
if __name__ == '__main__':
    ...
    # 2. Load and merge Pi Dataset per split
    if os.path.exists(config.pi_data_dir):
        print("Loading Pi dataset via load_speech_commands...")
        (x_pi_tr, y_pi_tr), (x_pi_v, y_pi_v), (x_pi_te, y_pi_te) = load_speech_commands(
        ...
        print("Processing Pi dataset splits...")
        x_pi_tr_proc = process_pi_split(x_pi_tr, config.dtype)
        x_pi_v_proc  = process_pi_split(x_pi_v, config.dtype)
        x_pi_te_proc = process_pi_split(x_pi_te, config.dtype)
        
        # Match dimensions with main dataset if needed
        if x_train.ndim == 3:
            if x_pi_tr_proc.ndim == 2: x_pi_tr_proc = x_pi_tr_proc[..., np.newaxis]
            if x_pi_v_proc.ndim == 2:  x_pi_v_proc  = x_pi_v_proc[..., np.newaxis]
            if x_pi_te_proc.ndim == 2: x_pi_te_proc = x_pi_te_proc[..., np.newaxis]

        # Oversample ONLY the training data
        print(f"Oversampling Pi training split by a factor of {config.pi_oversample_factor}...")
        x_pi_tr_oversampled = np.tile(x_pi_tr_proc, (config.pi_oversample_factor,) + (1,) * (x_pi_tr_proc.ndim - 1))
        y_pi_tr_oversampled = np.tile(y_pi_tr, config.pi_oversample_factor)
        
        # Merge splits: Train to Train, Val to Val, Test to Test
        x_train = np.concatenate([x_train, x_pi_tr_oversampled], axis=0)
        y_train = np.concatenate([y_train, y_pi_tr_oversampled], axis=0)
        
        x_val = np.concatenate([x_val, x_pi_v_proc], axis=0)
        y_val = np.concatenate([y_val, y_pi_v], axis=0)
        
        x_test = np.concatenate([x_test, x_pi_te_proc], axis=0)
        y_test = np.concatenate([y_test, y_pi_te], axis=0)
        
        print(f"Merged Training size:   {len(x_train)} samples")
        print(f"Merged Validation size: {len(x_val)} samples")
        print(f"Merged Test size:       {len(x_test)} samples")
    else:
        print("Pi dataset directory not found. Skipping Pi dataset integration.")

    # 3. Create Augmentation and tf.data.Dataset
    ...
    train_ds = train_ds.shuffle(8192)
